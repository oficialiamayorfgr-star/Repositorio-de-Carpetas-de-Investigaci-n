# Repositorio-de-Carpetas-de-Investigaci-n
...
body {font-family: Arial, sans-serif; background-color:#f4f4f4; overflow-x:hidden; position:relative;}
.header {background-color: #003366; color:white; text-align:center; padding:20px 0; box-shadow:0 4px 10px rgba(0,0,0,0.3); position:relative; overflow:hidden;}
.header img {width:100px; height:auto; animation:logoFade 2s ease;}
.header h1 {margin-top:10px; font-size:26px; animation:fadeIn 2s ease;}
...
.footer {background-color:#003366; color:white; text-align:center; padding:10px 0; margin-top:40px;}
.footer a {color:white; text-decoration:underline;}
.container {display:flex; justify-content:center; flex-wrap:wrap; gap:20px; padding:30px 20px; position: relative; z-index:1;}
.card {background:#fff; padding:30px 25px; border-radius:15px; box-shadow:0 8px 20px rgba(0,0,0,0.15); text-align:center; width:320px; opacity:0; transform:translateY(20px); animation:fadeInUp 1s forwards;}
input, select, button {display:block; margin:15px auto; padding:10px; width:90%; border:1px solid #ccc; border-radius:8px; font-size:14px;}
button {background:#0066cc; color:white; border:none; cursor:pointer; transition:0.3s;}
button:hover {background:#004d99;}
pre {text-align:left; max-height:400px; overflow:auto; background:#e6e6e6; padding:10px; border-radius:5px; white-space: pre-wrap;}
#actionResult a {color:#003366; text-decoration:none; font-weight:bold;}
#actionResult a:hover {text-decoration:underline;}
#addFolderForm {display:none; margin-top:15px;}
#processing {position: fixed; top:20px; right:20px; background: rgba(0,102,204,0.9); color:white; padding:10px 20px; border-radius:10px; display:none; align-items:center; gap:10px; font-weight:bold; z-index:1000;}
#processing .spinner {border:4px solid #f3f3f3; border-top:4px solid white; border-radius:50%; width:20px; height:20px; animation:spin 1s linear infinite;}
.square {position:absolute; width:50px; height:50px; opacity:0.2; border-radius:5px; pointer-events:none; animation:floatSquare linear infinite;}
...
@keyframes floatSquare {0% { transform: translateY(0px);} 50% { transform: translateY(-60px);} 100% { transform: translateY(0px);}}
...
<div id="menuDiv" class="card" style="display:none;">
  ...
  <label>Buscar Carpeta:</label>
  <input type="text" id="searchCarpeta" placeholder="Filtrar por número..." onkeyup="filtrarCarpetas()">
  ...
  <div id="addFolderForm" class="card">
  ...
  <p><strong>Miembros activos:</strong></p>
...
// Usuarios y roles
const users = {
  Darvin:{pass:"0497",role:"Agente"},
  Yahir:{pass:"3612",role:"Agente"},
  Emma:{pass:"4185",role:"Técnico"},
  Iker:{pass:"5193",role:"Fiscal"},
  Ct:{pass:"2907",role:"Agente"},
  Nicolas:{pass:"7359",role:"Técnico"},
  Gaspar:{pass:"6847",role:"Agente"},
  Emmanuel:{pass:"3495",role:"Agente"},
  Lego:{pass:"4697",role:"Técnico"},
  Jacob:{pass:"6520",role:"Fiscal"},
  Facundo:{pass:"0183",role:"Agente"},
  Angel:{pass:"3164",role:"Agente"},
  Paski:{pass:"4327",role:"Técnico"},
  ADMIN:{pass:"045689",role:"Administrador"}
};
...
// Carpetas
function cargarCarpetas(filter=""){
...
function actualizarMiembros(){
  ...
  activeUsers.forEach(u=>{
    const li=document.createElement("li"); li.innerText=u+" ("+users[u].role+")"; ul.appendChild(li);
  });
}

// Procesando
function showProcessing(duration=2000){
...
document.getElementById("loginForm").addEventListener("submit",async function(e){
  ...
  if(users[user] && users[user].pass===pass){
    ...
    if(!activeUsers.includes(user)) activeUsers.push(user); actualizarMiembros();
...
function showMenu(){
  ...
  document.getElementById("userRole").innerText=users[currentUser].role;
  cargarCarpetas();
...
// Abrir carpeta
function abrirLink(){
  const carpetaNum=document.getElementById("carpeta").value; const link=carpetas[carpetaNum];
  logAction(currentUser,"Accedió a Carpeta "+carpetaNum);
  showProcessing(5000).then(()=>{window.open(link,"_blank");});
  mostrarEstado();
}
...
function mostrarEstado(){
  ...
  if(currentUser==="ADMIN"){ document.getElementById("estadoSelect").value=estados[sel]; }
}
...
// Buscar carpeta
function filtrarCarpetas(){cargarCarpetas(document.getElementById("searchCarpeta").value.trim());}

// Logout
function logout(){logAction(currentUser,"Cierre de sesión"); activeUsers=activeUsers.filter(u=>u!==currentUser); actualizarMiembros(); currentUser=null; document.getElementById("menuDiv").style.display="none"; document.getElementById("loginDiv").style.display="block";}
...
function actualizarUsuariosParaAdmin(){
  ...
  Object.keys(users).forEach(u => {
    if(users[u].role!=="Administrador" && u!=="ADMIN"){ const opt=document.createElement("option"); opt.value=u; opt.text=u+" ("+users[u].role+")"; select.add(opt);}
  });
}
function darPoderesAdmin(){
  const selectedUser=document.getElementById("userToAdmin").value;
  if(selectedUser){users[selectedUser].role="Administrador"; logAction(currentUser,"Otorgó poderes de ADMIN a "+selectedUser); alert("Usuario "+selectedUser+" ahora tiene poderes de ADMIN."); actualizarUsuariosParaAdmin();}
}

// Figuras flotantes
for(let i=0;i<6;i++){
  const sq=document.createElement("div"); sq.className="square"; sq.style.backgroundColor=["#003366","#0066cc","#6699cc"][i%3];
  sq.style.left=Math.random()*window.innerWidth+"px";
  sq.style.top=Math.random()*window.innerHeight+"px";
  sq.style.animationDuration=(5+Math.random()*5)+"s"; document.body.appendChild(sq);
}
</script>
</body>
</html>
